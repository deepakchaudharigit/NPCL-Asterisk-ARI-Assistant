    ...
    try:
        ...
        while True:
            try:
                ...
                if user_input.lower() in ['quit', 'exit', 'bye', 'goodbye', 'बाहर निकलें', 'প্রস্থান']:
                    goodbye_messages = {
...
def start_offline_voice_mode(language_config, voice_handler=None):
    ...
    def get_offline_response(user_input, lang_code):
        ...
        if any(word in user_lower for word in ['hello', 'hi', 'namaste', 'नमस्ते']):
    ...
    while True:
        try:
            ...
            # Check for exit
            if user_input.lower() in ['quit', 'exit', 'bye', 'goodbye', 'बाहर निकलें']:
                goodbye_messages = {
                    "en-IN": "Thank you for contacting NPCL. Have a great day!",
                    "hi-IN": "एनपीसीएल से संपर्क करने के लिए धन्यवाद। आपका दिन शुभ हो!"
                }
